...
const AllRooms = () => {
    ...
    // Function to check if a room matches selected amenities
    const matchesAmenities = (room) => {
        return selectedFilters.amenities.length === 0 ||
            selectedFilters.amenities.every(a => room.amenities.includes(a));
    };
    ...
    const filteredRooms = useMemo(() => {
        return rooms
            .filter(room => matchesRoomType(room) && matchesPriceRange(room) && matchesAmenities(room) && filterDestination(room))
            .sort(sortRooms);
                ...
                <div className={`${openFilters ? "h-auto" : "h-0 lg:h-auto"} overflow-hidden transition-all duration-700`}>
                    ...
                    <div className='px-5 pt-5'>
                        ...
                        {amenityList.map((amenity, index) => (
                            <CheckBox key={index} label={amenity} selected={selectedFilters.amenities.includes(amenity)} onChange={(checked) => handleFilterChange(checked, amenity, 'amenities')} />
                        ))}
                    </div>
